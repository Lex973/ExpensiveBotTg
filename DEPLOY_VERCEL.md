...
1. Зарегистрируйтесь на <https://turso.tech/> и создайте базу. Запомните её
   регион: функция Vercel должна работать в том же регионе, иначе каждый из
   10–15 SQL-запросов на одно сообщение идёт через океан и ответ бота растягивается
   до 3–4 секунд. В `vercel.json` поле `regions` должно соответствовать базе:
   `aws-us-east-1` → `iad1`, `aws-eu-west-1` → `dub1`, `aws-eu-central-1` → `fra1`.
